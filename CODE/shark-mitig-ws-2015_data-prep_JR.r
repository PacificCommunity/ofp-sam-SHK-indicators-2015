...
# the ll_shark_sets_HWOB.txt is missing?
...
shkbait <- unlist(read.csv(paste(shkdir,"sharkbait.csv", sep=""))) # code for bait used for shark
...
#   sets<-sets2  because we can't find the raw for sets1
rm(sets1,sets2)
...
head(catch)
 ...
 temptab <- with(catch, table(l_set_id, sp_code ))
head(temptab)
...
#########################################################
#
#table the shark catch by sets (main species only.)
#
#
# BSH
tx <- with(catch[catch$sp_category=="BSH",], tapply(sp_category, l_set_id, table))
txx<- as.matrix(tx);
bsh <- as.data.frame( cbind( l_set_id=as.numeric(rownames(txx)), BSH=txx), nrow=length(txx), byrow=F , dimnames=list(NULL, c("l_set_id", "BSH")))
colnames(bsh) <- c("l_set_id", "BSH")
#
sets$BSH<-0
#
pntr <- match( bsh$l_set_id, sets$l_set_id)
sets$BSH[pntr] <- bsh$BSH[pntr]
#
rm(tx, txx, bsh)
#################################################
# FAL
...
sets$FAL<-0
#
pntr <- match( FAL$l_set_id, sets$l_set_id)
...
# OCS
#
tx <- with(catch[catch$sp_category=="OCS",], tapply(sp_category, l_set_id, table))
txx<- as.matrix(tx);
OCS <- as.data.frame( cbind( l_set_id=as.numeric(rownames(txx)), OCS=txx), nrow=length(txx), byrow=F , dimnames=list(NULL, c("l_set_id", "OCS")))
colnames(OCS) <- c("l_set_id", "OCS")
#
sets$OCS<-0
#
pntr <- match( OCS$l_set_id, sets$l_set_id)
sets$OCS[pntr] <- OCS$OCS[pntr]
head(sets) 
rm(tx, txx, OCS)
#
#################################################
# MAK
#
tx <- with(catch[catch$sp_category=="MAK",], tapply(sp_category, l_set_id, table))
txx<- as.matrix(tx);
MAK <- as.data.frame( cbind( l_set_id=as.numeric(rownames(txx)), MAK=txx), nrow=length(txx), byrow=F , dimnames=list(NULL, c("l_set_id", "MAK")))
colnames(MAK) <- c("l_set_id", "MAK")
#
sets$MAK<-0
#
pntr <- match( MAK$l_set_id, sets$l_set_id)
sets$MAK[pntr] <- MAK$MAK[pntr]
head(sets) 
rm(tx, txx, MAK)
#
#################################################
# THR
#
tx <- with(catch[catch$sp_category=="THR",], tapply(sp_category, l_set_id, table))
txx<- as.matrix(tx);
THR <- as.data.frame( cbind( l_set_id=as.numeric(rownames(txx)), THR=txx), nrow=length(txx), byrow=F , dimnames=list(NULL, c("l_set_id", "THR")))
colnames(THR) <- c("l_set_id", "THR")
#
sets$THR<-0
#
pntr <- match( THR$l_set_id, sets$l_set_id)
sets$THR[pntr] <- THR$THR[pntr]
head(sets) 
rm(tx, txx, THR)
#
#################################################
# HHD
#
tx <- with(catch[catch$sp_category=="HHD",], tapply(sp_category, l_set_id, table))
txx<- as.matrix(tx);
HHD <- as.data.frame( cbind( l_set_id=as.numeric(rownames(txx)), HHD=txx), nrow=length(txx), byrow=F , dimnames=list(NULL, c("l_set_id", "HHD")))
colnames(HHD) <- c("l_set_id", "HHD")
#
sets$HHD<-0
#
pntr <- match( HHD$l_set_id, sets$l_set_id)
sets$HHD[pntr] <- HHD$HHD[pntr]
head(sets) 
rm(tx, txx, HHD)

save(sets, file="C:/Projects/SHK-indicators-2015/DATA/ll_obs_sets_9June15.rdata")
#

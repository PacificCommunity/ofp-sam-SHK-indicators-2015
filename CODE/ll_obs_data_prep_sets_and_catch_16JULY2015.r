...
#llsethold     <- read.table(paste(dat.dir,"ll_shark_SET_HWOB.csv" , sep=''), sep="\t", header=T,  stringsAsFactors =F); nrow(llsethold) # like 800K
llseth     <- read.csv(paste(dat.dir,"ll_shark_SET_HWOB_13_July_2015.csv" , sep=''),   header=T,  stringsAsFactors =F); nrow(llseth) # 58169
...
llcatch_HW   <- read.csv(paste(dat.dir,  "ll_shark_catch_HWOB_13_july_2015.csv",sep=""),    header=T,  stringsAsFactors =F); dim(llcatch_HW) # 416138     13
...
#-----------------------------------final processing on the catch data
...
catch$program_code <- sets$program_code[pntr]
...
# Saving cleaned versions of catch and sets for analysis
  save(sets, catch,  file=paste0(dat.dir, "lldata_16JULY2015.rdata"))

...
The current study updates key indicators and extends the analyses to include hammerhead, porbeagle and \hlam{whale sharks}.
...
\section{Observed Species Composition Indicator Analyses}
 \subsection{Introduction}
The three other sets of Indicators used in this analysis - on distribution, CPUE and biological traits - focus on changes in each individual species isolated from the other shark species.  The Species Composition indicator focuses on patterns and trends in the relative composition of all shark species.  To the degree that observer samples of shark catches reflect changes in abundance, Species Composition indicators are a useful tool to monitor how fishing changes the community composition of sharks.  Another important reason for examining Species Composition indicators is to assess changes in the percentage of unidentified shark species over time.  Improvements in the observers' ability to identify sharks could contribute to increasing occurrences of species-specific records in the observer database and could bias temporal trends. 

The Species Composition Indicators are constructed by summing all shark catches in observer samples within each year and Region, subject to some data screening (described in \hl{Appendix X}).   For both longline and purse seine, three indicator time series are constructed for each region.  First, all longline and purse seine data are assembled and the proportion of each shark species, in terms of numbers, is computed (Annex figures SC\_01 and SC\_04, respectively).  To more finely evaluate species interactions, subsets of both gear type are constructed.  The longline data are parsed into shallow (10 or less hooks between floats) and deep ($>$ 10 hooks between floats) sets (Annex Figures \ref{SC_02} and \ref{SC_03}, respectively).  The purse seine data are divided between those associated with floating objects and unassociated (i.e., set on free schools) (Annex Figures \ref{SC_05} and \ref{SC_06}, respectively).  In addition to the shark species composition data, each of the Region plots contain info on the number of observed sets upon which the plots are based.  These effort data appear as bars above the composition data for each year; the large annual and spatial variability in observed sets is evident.

Given reliable data on overall sampling rates, and consideration of influences such as observer coverage, catchability and targeting, the overall proportion of sharks relative to target tuna species could similarly be described.  However, as the other indicators (particularly CPUE, as described in Section 5) more directly assess overall trends in abundance, we focus in this section solely on relative trends among the shark species.  Regardless of whether catch composition indicators are based on observer samples or the entire catch, changes in species composition over time can suggest relative population increases or depletions.
...
%------------------------------------------------------------------------------------------
 ...
 Silky sharks dominate purse seine shark bycatch, to the extent that other sharks are rarely seen (or tabulated) in recent years.  Oceanic whitetip sharks would appear to have been a much more common component of the shark bycatch up until the mid 2000s, however those data reflect an extremely low number of observed sets (see the effort bars atop the Region plots in Figure \ref{SC_02}).  As the number of observed sets began to increase substantially, the proportion of oceanic white tips declined to nearly zero.  It is unclear whether this reflects a true decline in oceanic whitetip sharks or a manner of purse seine fishing that resulted in an almost exclusive shark bycatch of silky sharks.  The only other shark species to occur as bycatch at greater than 1\% per year in any area are hammerhead sharks and the unidentified group of sharks, however such occurrences are sporadic.
 
In considering potential differences in shark bycatch related to purse seine set association, we restrict our analysis to Regions 3 and 4 as almost all purse seine fishing in other regions is dominated by associated sets with few observed sets (and perhaps few fished sets) occurring on free schools.  Silky shark dominate the shark bycatch in both associated and unassociated sets, particularly since the early 2000s.  The greatest difference between the two set types is an apparent slightly greater diversity of shark bycatch in unassociated sets, particularly with regard to oceanic whitetip sharks.  In any case, silky sharks have constituted more than 95\% of shark bycatch in both associated and unassociated sets over the past decade.
...
The Species Composition indicators reveal that shark bycatch differs substantially between longline and purse seine fishing in the WCPO.  Blue sharks are the most prevalent longline caught shark, but there are substantial regional and depth variations.  Several species are commonly caught more frequently in deeper sets; porbeagle form a sizable component of the shallow sets in Regions 5 and 6 and silky shark is the second most common longline caught shark.  Purse seine shark bycatch is much less variable and is dominated, particularly over the past decade when the number of observed sets increased greatly and composition data may have become more representative.  In virtually all regions and years, silky shark comprises more than 95\% of the shark bycatch, with minor numbers of hammerhead and oceanic whitetip sharks occurring.  Oceanic white tip sharks appear to have been more common prior to 2000, their percentage contribution to the overall shark cath has not been more than 20\% in regions 3 and 4 for over a decade, which is stark contrast to the first ten years of the study period. 
...
found significant declines in most combinations of spatial strata and sex for blue and mako sharks, as well as silky sharks.
...
The sex ratio of a shark population may also be a useful indicator of its status. Heavy exploitation could lead to a preferential loss of females because they tend to be larger and older than males.
...
\subsection{Methods}
%jr edits
\hlam{Sex specific length data from the longline observer data was was plotted by region for all key species} \hl{ the length plots, ANNEX :length}. 
Observed length  data from the purse seine fishery was limited to silky and oceanic whitetip sharks in regions 3 and 4.  In those instances that data was recorded in total length, these measurements were converted to fork length using conversion factors given in Table~\ref{tab:len_mat}. 
The median length by year along the annual 5th and 95th quantiles were plotted along with the length at maturity for each sex (Table~\ref{tab:len_mat}). Length data was plotted on a regionby region basis.  
 In the purse seine dataset, sexes were not usually recorded and only oceanic whitetip and silky sharks in Regions 3 and 4 had sufficient data for analysis (\ref{fig:ps_bio_len_FAL} )

Those 5x5 degree cells for which the sample size was less than 20 individuals were removed from the analysis. Results of the nominal analysis of size data for the longline fishery are shown in \hl{Annex 7}.%  this should be the plots bio_len_Female_BSH.png bio_len_Male_BSH.png and so on
...
To standardize for effects of space and observer coverage on the observed shark lengths generalized linear models were used to produce standardized annual lengths from the longline fishery. Difficulties with the standardizing purse seine data precluded the use of glm's on this data. 

A generalized linear model based on a normal distribution with factors year and 5x5 degree cell. The estimated model coefficients were used to predict shark lengths for each year for an arbitrarily chosen cell lying near the centre of each each population. 
To more adequeately capture the population trend this analysis was carried out  stock  level well as a   region by region analysis for the longline fisheries.
The stocks identified by  are; north and south pacific stocks for blue and mako sharks, a single stock for silky,  oceanic whitetip, thresher, hammerhead, and porbeagle sharks.   Models analyzing the purse seine data combined regions 3 and 4 used the annual median observed length. 
Summaries of the  results from the standardized  length trends  were created by fitting linear models year  coefficients from the GLM. 
The slopes of these linear models were used to identify significant trends in  lengths over time. 
 
Models were run separately for each species, sex, stock (for blue and mako sharks) on and fishery. One important
...
decreases in the later part of the time series may be characterized as having no trend through time.
...
Porbeagle sharks were only observed in regions 5 and 6, where nearly every female and the majoity of the male shaks were immature.  Increasing median lenght for both sexes were were observedin region 5 since 2007, Region 6 showed no consitent trends.



Standardized length compositions are shown in \hlam{Annex: Standardized Lengths. }, 

The only sharks that showed an increase in length over time at the population level (Figure~\ref{fig:len_coef} ) was mako shark in the  North pacific. regional analysis (Figure~\ref{fig:len_coef_reg} indicates that this is driven by rgion 3. WIth the exception of female  threshers in region 6, and male silky and oceanic whitetip sharks all showed decreases in length over time based on a linear model fit to the sandardized annual lengths. A summary of the results of the linear model fits by species, sex  is shown in and Figure~\ref{fig:len_coef_reg}.

Observed lenghts for silky and oceanic whitips in the purse siene fishery were explored via linear model on the observed lengths this inicated that the observed length of declined  over the study period for both species at the WCPO level and at the regional level (Regions 3 and 4 only) that observed declines existed for silky sharks in region 3 and 4 and oceanic whitetip in region 4. Observed lengths for oceanic whitetip int the purse seine fishery had a small positive slope of 0.003.
...
\addcenterfig[Observed sex ratio in the longline fishery, by region for key shark species, the region specific sample size is listed in the inset.]{fig:cpue_ps_an1}{LLSexRatio} 
...
\addcenterfig[The slopes of linear models fit to year coefficients produced by standardization models of lengths recorded in longline   and purse seine observer samples. For blue (BSH), mako (MAK), oceanic whitetip (OCS), silky (FAL) and thresher (THR) sharks. Positive and negative slopes lie above and below the dotted line.]{fig:len_coef}{len_stdz_coef_out} 

\addcenterfig[The slopes of linear models fit to year coefficients produced by standardization models of lengths recorded in longline   and purse seine observer samples. For blue (BSH), mako (MAK), oceanic whitetip (OCS), silky (FAL) and thresher (THR) sharks. Positive and negative slopes lie above and below the dotted line.]{fig:len_coef_reg}{LL_stdLEN_lm_coef_regionPOR_F} 

\subsection{Conclusions}
      
This section looked at trends in the observed sex ratio and length composition in the longline purse seine fishery over time.  The results of the sex ratio indicator analysis show that with a few exceptions (blue shark in region5) the observed sex ratio in the longline fishery is approximately equal.   

Observed length was investegated by plotting the nomial median length trends  and length at maturity at the regional level.  This indicated that the majority of the observed hammerhead, silky, thresher, oceanic whitetip and porbeagle sharks were immature. Observed blue shark were mainly immature in regions 5 and 6 and mainly mature in region 2.  

The results of the length standardization indicate declines in most regions for most speciesbased on a linear model fit to the sandardized annual lengths.  These results  mirror the larger scale popuation trends in length for the longline fishery and the popoulation level trends in silky and oceanic whitetip sharks in the purse seine fishery.  
...
\section{Conclusions}
% nb this part has been updated with new percentages, but the words are straight out to 2011, text is still highlighted becausee the data  'issues' are the exact same.....inital text is from the intro by HA
The current study updates key indicators and extends previousthe analyses to include hammerhead, porbeagle and whale sharks. We present information on the geographic range of catches for each of the species considered; temporal trends in catch composition and catch rates, and key biological indicators of fishing pressure such as mean size and sex ratio by species. Whale sharks are assessed separately due to the unique nature of their interactions with fisheries in the WCPO. The analyses are based on Secretariat of the Pacific Community - Oceanic Fisheries Programme (SPC-OFP) data holdings for sharks taken in longline and purse seine fisheries in the Western and Central Pacific Ocean (WCPO). The framework for the analysis is a series of indicators of fishing pressure and stock status that were first described in the Shark Research Plan presented to the sixth meeting of the Western and Central Pacific Fisheries Commission's (WCPFC) Scientific Committee (SC6; Clarke and Harley 2010). A preliminary indicator-based analysis of SPC data holdings was presented to the Commission in December 2010 (Clarke et al. 2010) with an exhaustive review of the fisheries and data sources presented to SC7 (Clarke et al. 2011).



\hl{This paper examines  data held by the SPC-OFP for longline and purse seine fisheries in the WCPO to make infrence regarding the   populations of key shark species in the WCPO.  
...
The biological indicators showed that   with a few exceptions (blue shark in region5) the observed sex ratio in the longline fishery is approximately equal. Sex ratio analysis was not possible for the purse seine fishery. Observed length was investegated by plotting the nomial median length trends  and length at maturity at the regional level.  This indicated that the majority of the observed hammerhead, silky, thresher, oceanic whitetip and porbeagle sharks were immature. Observed blue shark were mainly immature in regions 5 and 6 and mainly mature in region 2.   The results of the length standardization indicate declines in most regions for most speciesbased on a linear model fit to the sandardized annual lengths.  These results  mirror the larger scale popuation trends in length for the longline fishery and the popoulation level trends in silky and oceanic whitetip sharks in the purse seine fishery.  
Overall the biological indicators showed that by in large the longline fishery is interacting with smaller immature sharks of both sexs. 

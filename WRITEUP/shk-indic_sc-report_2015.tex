...
The Species Composition Indicators are constructed by summing all shark catches in observer samples within each year and Region, subject to some data screening (described in \hl{Appendix X}).   For both longline and purse seine, three indicator time series are constructed for each region.  First, all longline and purse seine data are assembled and the proportion of each shark species, in terms of numbers, is computed (Annex figures SC_01 and SC_04, respectively).  To more finely evaluate species interactions, subsets of both gear type are constructed.  The longline data are parsed into shallow (10 or less hooks between floats) and deep (> 10 hooks between floats) sets (Annex Figures SC_02 and SC_03, respectively).  The purse seine data are divided between those associated with floating objects and unassociated (i.e., set on free schools) (Annex Figures SC_05 and SC_06, respectively).  In addition to the shark species composition data, each of the Region plots contain info on the number of observed sets upon which the plots are based.  These effort data appear as bars above the composition data for each year; the large annual and spatial variability in observed sets is evident.
...
%------------------------------------------------------------------------------------------
 ...
 Silky sharks dominate purse seine shark bycatch, to the extent that other sharks are rarely seen (or tabulated) in recent years.  Oceanic whitetip sharks would appear to have been a much more common component of the shark bycatch up until the mid 2000s, however those data reflect an extremely low number of observed sets (see the effort bars atop the Region plots in Figure SC_02).  As the number of observed sets began to increase substantially, the proportion of oceanic white tips declined to nearly zero.  It is unclear whether this reflects a true decline in oceanic whitetip sharks or a manner of purse seine fishing that resulted in an almost exclusive shark bycatch of silky sharks.  The only other shark species to occur as bycatch at greater than 1\% per year in any area are hammerhead sharks and the unidentified group of sharks, however such occurrences are sporadic.
